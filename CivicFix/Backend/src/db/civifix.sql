...
DECLARE
    ...
    departamento_id INT;
BEGIN
    INSERT INTO Municipalidad (nombre, direccion, telefono, correo)
    VALUES ('Municipalidad Demo', 'Ciudad de Guatemala', '22223333', 'demo@civicfix.com')
    ON CONFLICT DO NOTHING;

    SELECT id_municipalidad INTO municipalidad_id
    FROM Municipalidad
    WHERE correo = 'demo@civicfix.com';

    INSERT INTO DepartamentoMunicipal (nombre, descripcion, fk_id_municipalidad)
    SELECT 'Atencion ciudadana', 'Departamento para pruebas de CivicFix', municipalidad_id
    WHERE NOT EXISTS (
        SELECT 1 FROM DepartamentoMunicipal
        WHERE nombre = 'Atencion ciudadana' AND fk_id_municipalidad = municipalidad_id
    );

    SELECT id_departamento INTO departamento_id
    FROM DepartamentoMunicipal
    WHERE nombre = 'Atencion ciudadana' AND fk_id_municipalidad = municipalidad_id;

    INSERT INTO Usuario (nombre, apellido, usuario, correo, password, telefono)
    VALUES ('Ciudadano', 'Demo', 'ciudadano.demo', 'ciudadano.demo@example.com',
        '$2b$10$hprvJ799.VNgJdwu1Z.h3.t.MZsy913qJVRB1uciPkXZK6JwnRroy', '55554444')
    ON CONFLICT DO NOTHING;

    INSERT INTO EmpleadoMunicipal (nombre, apellido, usuario, password, dpi, telefono, direccion, correo, cargo, id_departamento, id_municipalidad)
    VALUES ('Empleado', 'Demo', 'empleado.demo',
        '$2b$10$P9jfauxXczzdR0sAR770wuPMnHgtB9HU6JL3dm4mPk.yMlxfngDbC',
        '1234567890101', '55556666', 'Ciudad de Guatemala', 'empleado.demo@civicfix.com',
        'Empleado', departamento_id, municipalidad_id)
    ON CONFLICT DO NOTHING;

    INSERT INTO EmpleadoMunicipal (nombre, apellido, usuario, password, dpi, telefono, direccion, correo, cargo, id_departamento, id_municipalidad)
    VALUES ('Administrador', 'Demo', 'admin.demo',
        '$2b$10$BsDabARpfKjBPVCa6Xh1OuSsVN7zkuhvuX5zXwtvV6hGt7P3h2m.2',
        '1234567890102', '55557777', 'Ciudad de Guatemala', 'admin.demo@civicfix.com',
        'Administrador', departamento_id, municipalidad_id)
    ON CONFLICT DO NOTHING;
END $$;
