...
CREATE TABLE DepartamentoMunicipal (
    ...
    CONSTRAINT fk_dept_muni FOREIGN KEY (id_municipalidad) REFERENCES Municipalidad(id_municipalidad) ON DELETE CASCADE
);
...
CREATE TABLE ServicioMunicipal (
    ...
    CONSTRAINT fk_servicio_dept FOREIGN KEY (id_departamento) REFERENCES DepartamentoMunicipal(id_departamento) ON DELETE CASCADE
);
...
CREATE TABLE Reporte (
    ...
    CONSTRAINT fk_rep_tipo FOREIGN KEY (id_tipo_incidencia) REFERENCES TipoIncidencia(id_tipo_incidencia),
    CONSTRAINT fk_rep_ubicacion FOREIGN KEY (id_ubicacion) REFERENCES Ubicacion(id_ubicacion),
    CONSTRAINT fk_rep_estado FOREIGN KEY (id_estado) REFERENCES Estado(id_estado),
    CONSTRAINT fk_rep_prioridad FOREIGN KEY (id_prioridad) REFERENCES Prioridad(id_prioridad),
    CONSTRAINT fk_rep_servicio FOREIGN KEY (id_servicio) REFERENCES ServicioMunicipal(id_servicio)
);
...
CREATE TABLE FotografiaProblema (
    ...
    CONSTRAINT fk_fotoprob_reporte FOREIGN KEY (id_reporte) REFERENCES Reporte(id_reporte) ON DELETE CASCADE
);
...
CREATE TABLE EvidenciaSolucion (
    ...
    descripcion VARCHAR(255),
    fecha_subida TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_evidsol_reporte FOREIGN KEY (id_reporte) REFERENCES Reporte(id_reporte) ON DELETE CASCADE
);
...
CREATE TABLE Asignacion (
    ...
    CONSTRAINT fk_asig_reporte FOREIGN KEY (id_reporte) REFERENCES Reporte(id_reporte) ON DELETE CASCADE,
    CONSTRAINT fk_asig_empleado FOREIGN KEY (id_empleado) REFERENCES EmpleadoMunicipal(id_empleado)
);
...
CREATE TABLE Notificacion (
    ...
    CONSTRAINT fk_notif_reporte FOREIGN KEY (id_reporte) REFERENCES Reporte(id_reporte) ON DELETE CASCADE
);
...
CREATE TABLE BitacoraCambioEstado (
    ...
    CONSTRAINT fk_bit_reporte FOREIGN KEY (id_reporte) REFERENCES Reporte(id_reporte) ON DELETE CASCADE,
    CONSTRAINT fk_bit_est_ant FOREIGN KEY (id_estado_anterior) REFERENCES Estado(id_estado),
    CONSTRAINT fk_bit_est_nuev FOREIGN KEY (id_estado_nuevo) REFERENCES Estado(id_estado),
    CONSTRAINT fk_bit_empleado FOREIGN KEY (id_empleado) REFERENCES EmpleadoMunicipal(id_empleado)
);
...
CREATE INDEX idx_reporte_estado ON Reporte(id_estado);
...
INSERT INTO DepartamentoMunicipal (nombre, descripcion, id_municipalidad) VALUES
...
INSERT INTO ServicioMunicipal (nombre, descripcion, id_departamento) VALUES
...
INSERT INTO Reporte (titulo, descripcion, id_usuario, id_tipo_incidencia, id_ubicacion, id_estado, id_prioridad, id_servicio) VALUES
...
INSERT INTO FotografiaProblema (id_reporte, ruta_fotografia, descripcion) VALUES
...
INSERT INTO Asignacion (id_reporte, id_empleado, observacion) VALUES
(1, 1, 'Cuadrilla programada para reparación en horario nocturno.');

INSERT INTO BitacoraCambioEstado (id_reporte, id_estado_anterior, id_estado_nuevo, id_empleado, comentario) VALUES
...
INSERT INTO EvidenciaSolucion (id_reporte, ruta_fotografia, descripcion) VALUES
(1, 'https://storage.civicfix.com/uploads/bache_reparado_z1.jpg', 'Trabajos de bacheo y nivelación completados');

INSERT INTO Notificacion (id_usuario, id_reporte, titulo, mensaje, leida) VALUES
