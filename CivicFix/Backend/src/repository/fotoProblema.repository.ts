...
export class FotoProblemaRepository {
    async obtenerTodos(): Promise<FotografiaProblema[]> {
        const resultado = await pool.query<FotografiaProblema>(`SELECT ${COLUMNAS} FROM FotografiaProblema ORDER BY id_fotografia`);
        return resultado.rows;
    }

    async obtenerPorId(id: number): Promise<FotografiaProblema | null> {
        const resultado = await pool.query<FotografiaProblema>(`SELECT ${COLUMNAS} FROM FotografiaProblema WHERE id_fotografia = $1`, [id]);
        return resultado.rows[0] ?? null;
    }

    async obtenerPorReporte(idReporte: number): Promise<FotografiaProblema[]> {
        const resultado = await pool.query<FotografiaProblema>(`SELECT ${COLUMNAS} FROM FotografiaProblema WHERE fk_id_reporte = $1 ORDER BY fecha_subida`, [idReporte]);
        return resultado.rows;
    }
    ...
    async crear(foto: FotografiaProblema): Promise<FotografiaProblema> {
        const resultado = await pool.query<FotografiaProblema>(
            ...
            [foto.fk_id_reporte, foto.ruta_fotografia, foto.descripcion, foto.fecha_subida ?? new Date()]
        );
    ...
    async actualizar(id: number, foto: FotografiaProblema): Promise<FotografiaProblema | null> {
        const resultado = await pool.query<FotografiaProblema>(
            `UPDATE FotografiaProblema
             ...
             WHERE id_fotografia = $5 RETURNING ${COLUMNAS}`,
    ...
    async eliminar(id: number): Promise<FotografiaProblema | null> {
        const resultado = await pool.query<FotografiaProblema>(`DELETE FROM FotografiaProblema WHERE id_fotografia = $1 RETURNING ${COLUMNAS}`, [id]);
        return resultado.rows[0] ?? null;
    }
}
